...
func TestInitServiceSuccessRegistersAndStartsWorker(t *testing.T) {
	...
	createTemporalWorker = func(_ temporalClient, taskQueue string, _ worker.Options) (temporalWorker, error) {
	...
	if gotTaskQueue != "feeworker" {
		t.Fatalf("worker task queue = %q, want feeworker", gotTaskQueue)
	}
	...
	if got := reflect.TypeOf(fakeWorker.activities[0]).String(); got != "*feeworker.Activities" {
	...
	if registeredActivities.billSealClient == nil {
		t.Fatal("registered activities has no Fees seal client")
	}
