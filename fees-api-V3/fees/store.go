...
func readBillMetadata(ctx context.Context, id string) (*BillResource, error) {
	return readBillMetadataFrom(ctx, db, id)
}

func persistOpenBillResource(ctx context.Context, input BillInput) (*BillResource, bool, error) {
	...
	err := db.QueryRow(ctx, `
		...
		RETURNING bill_id, client_id, currency, period, status, opened_at, closed_at`,
		id,
		input.ClientID,
		input.Currency,
		input.Period,
	...
	if err == nil {
		resourcePtr, readErr := readBillResource(ctx, id)
		if readErr != nil {
			return nil, false, fmt.Errorf("read persisted open bill %s: %w", id, readErr)
		}
		return resourcePtr, true, nil
	}
	if !errors.Is(err, sqldb.ErrNoRows) {
		return nil, false, fmt.Errorf("persist open bill %s: %w", id, err)
	}

	resourcePtr, err := readBillResource(ctx, id)
	if err != nil {
		return nil, false, fmt.Errorf("read bill %s after insert conflict: %w", id, err)
	}
	return resourcePtr, false, nil
}
